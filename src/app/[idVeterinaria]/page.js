...
// ==========================================
// 1. LÓGICA DE CARGA DE CLÍNICA (HOOK)
// ==========================================
const useClientHomeLogic = (idVeterinaria) => {
  ...
  useEffect(() => {
    const cargarDatos = async () => {
      try {
        const docRef = doc(db, "clinicas", idVeterinaria);
        const snap = await getDoc(docRef);
        
        if (snap.exists()) {
          setClinica(snap.data());
        } else {
          setClinica({ nombre: idVeterinaria.replace('-', ' ').toUpperCase() });
        }
      } catch (error) {
    ...
    if (idVeterinaria) cargarDatos();
  }, [idVeterinaria]);

  return { clinica, cargando };
};

// ==========================================
// 2. VISTA PRINCIPAL (FRONTEND)
// ==========================================
export default function DynamicClientHome({ params }) {
  const { idVeterinaria } = use(params);
  const logic = useClientHomeLogic(idVeterinaria);

  // --- ESTADOS DE FLUJO DEL CLIENTE ---
  ...
  // --- MANEJO DE NAVEGACIÓN NATIVA ---
  useEffect(() => {
    const handlePopState = () => {
      if (vista !== 'menu') cerrarModal(false);
      else if (viendoCartilla || viendoHistorial) {
        setViendoCartilla(false);
        setViendoHistorial(false);
      }
    };
    window.addEventListener('popstate', handlePopState);
    return () => window.removeEventListener('popstate', handlePopState);
  }, [vista, viendoCartilla, viendoHistorial]);
  ...
  const buscarMascotas = async () => {
    if (!telefono || telefono.length < 8) return alert("Ingresa un número válido");
    setBuscando(true);
    try {
    ...
    } catch (e) { alert("Error al buscar"); } finally { setBuscando(false); }
  };

  const guardarEdicion = async () => {
    if (!mascotaEditando.nombre) return alert("Nombre obligatorio");
    setGuardando(true);
    try {
      await updateDoc(doc(db, "pacientes", mascotaEditando.id), {
        nombre: mascotaEditando.nombre, especie: mascotaEditando.especie,
        raza: mascotaEditando.raza, edad: mascotaEditando.edad
      });
  ...
  // --- PANTALLAS DE CARGA Y VISUALIZADORES ---
  if (logic.cargando) return <div className="min-h-screen flex items-center justify-center bg-gray-50 dark:bg-slate-900"><div className="animate-pulse text-blue-600 font-bold">Cargando...</div></div>;
  if (viendoCartilla && mascotaSeleccionada) return <CartillaClienteView mascota={mascotaSeleccionada} onBack={() => setViendoCartilla(false)} />;
  if (viendoHistorial && mascotaSeleccionada) return <HistorialClienteView mascota={mascotaSeleccionada} onBack={() => setViendoHistorial(false)} />;

  const nombreClinica = logic.clinica?.nombre || "Veterinaria";

  return (
    <div className="min-h-screen flex flex-col overflow-hidden relative bg-white dark:bg-slate-900">
       
       {/* 1. FONDO */}
       <div className="fixed inset-0 z-0 pointer-events-none">
          <div className="absolute inset-0 bg-cover bg-center opacity-20 dark:opacity-10" style={{ backgroundImage: `url('${logic.clinica?.fondoUrl || '/bg-cliente.jpg'}')` }}></div>
          <div className="absolute inset-0 bg-gradient-to-b from-white/10 via-white/80 to-gray-50 dark:from-slate-900/10 dark:via-slate-900/80 dark:to-slate-900"></div>
       </div>
       ...
       <div className="relative z-10 bg-gradient-to-br from-blue-600/95 to-purple-600/95 backdrop-blur-md pb-24 pt-12 px-6 rounded-b-[3rem] shadow-xl animate-slide-up-fade">
          <header className="relative z-10 text-center sm:text-left">
             <h1 className="text-3xl font-extrabold text-white mb-1 drop-shadow-sm">¡Hola! 👋</h1>
             <p className="text-blue-100 text-lg font-medium drop-shadow-sm">Bienvenido a {nombreClinica}</p>
          </header>
          {/* Decoración */}
       ...
       <div className="flex-1 px-6 -mt-16 relative z-10 pb-8">
         <div className="grid grid-cols-1 gap-4">
           ...
           {/* Botón Agendar (Ruta Dinámica) */}
           <Link 
              href={`/${idVeterinaria}/agendar`} 
              className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-6 rounded-3xl shadow-lg border border-white/50 dark:border-slate-700 flex items-center gap-5 hover:bg-white dark:hover:bg-slate-800 hover:scale-[1.02] hover:shadow-xl transition-all duration-300 group active:scale-95 animate-slide-up-fade"
           ...
           >
              ...
              <div className="flex-1">
                ...
                <p className="text-sm text-gray-600 dark:text-gray-300 mt-1 leading-tight font-medium">Agendar visita con nosotros</p>
              </div>
           ...
           <div className="grid grid-cols-2 gap-4 animate-slide-up-fade" style={{ animationDelay: '200ms' }}>
              ...
              <button 
                  ...
                  className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-5 rounded-3xl shadow-md border border-white/50 dark:border-slate-700 flex flex-col items-center justify-center text-center gap-3 hover:bg-white dark:hover:bg-slate-800 hover:shadow-lg transition-all duration-300 group active:scale-95"
              >
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-gray-500 dark:text-gray-400 mt-1 leading-tight font-medium">
                        Ver historial
                    </p>
                  </div>
              </button>
              
              {/* Botón Llamar (Podrías hacerlo dinámico si guardas el teléfono en la colección 'clinicas') */}
              <a href={`tel:${logic.clinica?.telefono || '5555555555'}`} className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-md p-5 rounded-3xl shadow-md border border-white/50 dark:border-slate-700 flex flex-col items-center justify-center text-center gap-3 hover:bg-white dark:hover:bg-slate-800 hover:shadow-lg transition-all duration-300 group active:scale-95">
           ...
           <div className="mt-4 bg-orange-50/95 dark:bg-slate-800/90 backdrop-blur-md p-5 rounded-3xl border border-orange-100 dark:border-orange-900/30 flex items-start gap-4 shadow-sm animate-slide-up-fade" style={{ animationDelay: '300ms' }}>
              ...
              <div>
                 ...
                 <p className="text-xs text-orange-800/80 dark:text-gray-300 leading-relaxed font-medium">
                   Mantén sus vacunas al día. {nombreClinica} te ayuda a recordarlo.
                 </p>
              </div>
           </div>
         </div>
       ...
       {/* ============================================== */}
       {mascotaSeleccionada && (
           <div className="fixed inset-0 z-[60] bg-black/60 backdrop-blur-sm flex items-end sm:items-center justify-center animate-page" onClick={() => setMascotaSeleccionada(null)}>
               <div className="bg-white dark:bg-slate-900 w-full sm:w-96 rounded-t-3xl sm:rounded-3xl p-6 shadow-2xl animate-pop-up" onClick={e => e.stopPropagation()}>
       ...
       {/* ============================================== */}
       {mascotaEditando && (
           <div className="fixed inset-0 z-[70] bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 animate-page">
